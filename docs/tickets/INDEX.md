...
Decomposition of PRD.md §8 Units 0–14 into 24 leaf-first tickets. PRD units
...
Open, unpinned decisions live in [OPEN-DECISIONS.md](OPEN-DECISIONS.md) —
do not let builders invent them.
...
|---|---|---|---|
| [domain-models](domain-models.json) | §5, tuning file | standard | — |
...
| [audio-playback](audio-playback.json) | Unit 13 (app half) | standard | domain-models |
| [analytics](analytics.json) | Unit 12 | hard | domain-models |
...
| Ticket | PRD | Tier | Deps |
|---|---|---|---|
| [word-matcher](word-matcher.json) | Unit 4 (matching core) | **intensive** | domain-models, listening-contracts |
| [word-state-machine](word-state-machine.json) | Unit 5 (logic half) | standard | domain-models, listening-contracts |
| [pack-build-cli](pack-build-cli.json) | Unit 3 (CLI; Dart chosen) | hard | domain-models, decodability-linter |
...
| Ticket | PRD | Tier | Deps |
|---|---|---|---|
| [listening-tracker](listening-tracker.json) | Unit 4 (tracker/fallback/cap) | hard | domain-models, listening-contracts, word-matcher |
...
| [content-delivery](content-delivery.json) | Unit 11 | hard | domain-models, pack-build-cli |
...
| [twister-flow](twister-flow.json) | Unit 14 | hard | design-tokens, domain-models, listening-contracts, word-matcher, word-state-machine, audio-playback, local-storage, analytics |

### Wave 5 — integration

| Ticket | PRD | Tier | Deps |
|---|---|---|---|
| [app-shell](app-shell.json) | Unit 1 (shell) + wiring | standard | design-tokens, reading-screen, progress-map-collection, profiles-parent-corner, vocab-cards, celebration-sequence, twister-flow, content-delivery, analytics, listening-tracker, stuck-word-scaffold |

## Orchestrator notes

- **pubspec (central):** add an audio playback plugin (OD-8 proposes
  `just_audio`); optionally `fake_async`/`clock` as explicit dev_dependencies
...
- **External blockers:** `platform-asr-adapter` blocks on the owner-run
  Unit 0 spike verdict. Real content/assets (fonts, .riv, recordings,
  scope-&-sequence table) are owner deliverables (OD-3, OD-7, OD-10); all
  tickets run on generated fixtures until then.
- **PRD-unit → ticket map:** U0→spike-asr-app; U1→design-tokens+app-shell;
  ...
  U14→twister-flow (+pack validation in pack-build-cli). §5 models →
  domain-models + local-storage.
